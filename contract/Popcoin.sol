...
contract EllipticCurve {
    ...
    //     return sha256(abi.encodePacked(msg.sender, "zhq"));
    // }
    
    // function Verify(uint[2] memory rs, uint[2] memory Q) public view returns (bool) {
    //     return validateSignature(hash(), rs, Q);
    // }
}
...
struct TransactionWithIdentity {
    address from;
    address to;
...
contract Popcoin is ERC20Interface, SafeMath, EllipticCurve {
    string public name;
    string public symbol;
    uint8 public decimals;
    uint32 public basicIncome;
    uint[2] Q;

    uint256 public _totalPopletsSupply;
    uint256 public _exchangeRate;

    mapping(address => uint) balances;
    mapping(address => string) identifiers;
    ...
    constructor() public {
        ...
        //emit Transfer(address(0), msg.sender, _totalPopletsSupply);
        
        members.push(msg.sender);
    }

    function totalSupply() public view virtual override returns (uint) {
        return _totalPopletsSupply - balances[address(0)];
    }

    function balanceOf(address tokenOwner) public view virtual override returns (uint) {
        return safeDiv(balances[tokenOwner], _exchangeRate);
    }
    
    function balanceOfPoplets(address tokenOwner) public view virtual returns (uint) {
    ...
    function record(address from, address to, uint tokens) public {
        ...
        transactions[from].push(t);
        transactions[to].push(t);
    }
    
    function getTransactions(address addr, uint start, uint end) public view returns (TransactionWithIdentity[] memory) {
        require(end>start, "start must be less than end");
        uint l=start;
        uint r=end;
        if (r>transactions[addr].length) r=transactions[addr].length;
        ...
        for (uint i = l; i < r; i++)
        {
            ...
            uint amount = transactions[addr][i].amount;
            uint timestemp = transactions[addr][i].timestemp;
            TransactionWithIdentity memory t = TransactionWithIdentity({
                from : from,
                to : to,
                amount : amount,
                timestemp : timestemp,
                fromIdentifier : identifiers[from],
    ...
    function transfer(address to, uint tokens) public virtual override returns (bool success) {
        //require(tokens > 0, "The amount needs to be more than 0.");
        if (tokens == 0) return false;
        if (balances[msg.sender] < safeMul(tokens, _exchangeRate)) return false;
        balances[msg.sender] = safeSub(balances[msg.sender], safeMul(tokens, _exchangeRate));
        balances[to] = safeAdd(balances[to], safeMul(tokens, _exchangeRate));
        record(msg.sender, to, tokens);
        emit Transfer(msg.sender, to, tokens);
        return true;
    }

    function transferFrom(address from, address to, uint tokens) public virtual override returns (bool) {
        balances[from] = safeSub(balances[from], safeMul(tokens, _exchangeRate));
        allowed[from][msg.sender] = safeSub(allowed[from][msg.sender], tokens);
        balances[to] = safeAdd(balances[to], safeMul(tokens, _exchangeRate));
        record(from, to, tokens);
    ...
    //To Do: Expire Time
    function addMember(address member, string memory identity, uint[2] memory rs) public returns (bool) {
        require(isMember(member) == false, "The address has already added");
        bytes32 hash = sha256(abi.encodePacked(member, identity));
        if (validateSignature(hash, rs, Q) == false)
            return false;
        members.push(member);
        identifiers[member] = identity;
        return true;
    }

    //To Do: Automatic Trigger, Expire Users
    function newPeriod() public returns (uint) {
        uint256 newTotalPopletsSupply = safeDiv(safeMul(_totalPopletsSupply, 50), 49);
        uint256 newCreatedPoplets = safeSub(newTotalPopletsSupply, _totalPopletsSupply);

        uint256 length = members.length;

        uint256 incomePoplets = safeDiv(newCreatedPoplets, length);
        uint256 remainder = safeSub(newCreatedPoplets, safeMul(incomePoplets, length));
        
        _exchangeRate = safeDiv(newCreatedPoplets, safeMul(basicIncome, length));

        for (uint i = 0; i < length; i++) {
            balances[members[i]] += incomePoplets;
            record(address(0), members[i], safeDiv(incomePoplets, _exchangeRate));
            emit Transfer(address(0), members[i], safeDiv(incomePoplets, _exchangeRate));
        }
        balances[members[0]] += remainder;
        record(address(0), members[0], safeDiv(remainder, _exchangeRate));
        emit Transfer(address(0), members[0], safeDiv(remainder, _exchangeRate));

        _totalPopletsSupply = newTotalPopletsSupply;
        return _exchangeRate;
    }
}
