...
fdescribe("Meetup Test", () => {
    fdescribe("Meetup", async () => {
        ...
        it("must store meetups and give back list", async () => {
            ...
            meetups = await phrpc.meetups(new Meetup(user1.credentialInstance.credential, "somewhere"));
            expect(meetups.length).toEqual(1);
            meetups = await phrpc.meetups(new Meetup(user1.credentialInstance.credential, "somewhere"));
            expect(meetups.length).toEqual(1);

            Log.lvl2("verify second user can join");
            meetups = await phrpc.meetups(new Meetup(user2.credentialInstance.credential, "somewhere"));
